"""
SUBIT-T Router - operator-driven agent dispatch.
...
class Router:
    ...
    def route(
        ...
        """
        Apply an operator to the current state and dispatch to an agent.

        Returns a record with transition metadata and the agent result.
        """
        ctx = context or {}
        transition = current.apply(op)

        fn = (
            self._by_state_op.get((transition.result.bits, op.value))
            or self._by_state.get(transition.result.bits)
            or self._by_op.get(op.value)
            or self._fallback
        )

        agent_result = fn(transition.result, op, ctx) if fn else None
        record = {
    ...
    def route_text(
        ...
        Returns a record with encoding metadata, transition metadata, and agent output.
        """
        encoded = encode(text)
        record = self.route(encoded.current_state, encoded.operator, context)
        record["encoding"] = encoded.to_dict()
        return record
    ...
    def chain(
        ...
        log = []
        for op in ops:
            record = self.route(current, op, ctx)
            log.append(record)
            current = State(record["transition"]["result"]["bits"])
        return log
    ...
    def state_distribution(self) -> dict[str, int]:
        counts: dict[str, int] = {}
        for record in self._history:
            name = record["transition"]["result"]["name"]
            counts[name] = counts.get(name, 0) + 1
        return counts

    def stuck_detection(self) -> dict[str, bool]:
        """Detect suspicious routing skews in v3 history."""
    ...
    def idempotent_rate(self) -> float:
        ...
        if not total:
            return 0.0
        return sum(1 for record in self._history if record["transition"]["idempotent"]) / total
