"""
SUBIT-T Injector — (State, Op) → system prompt.
...
from .canon import WHO_LABEL, WHERE_LABEL, WHEN_LABEL
...
def build_prompt(
    ...
    """
    Build system prompt for an LLM agent in archetypal state.

    Args:
        state:      the resulting state after operator is applied
        op:         the operator that produced this state
        user_input: original user text (included for context)
        extra:      any additional instructions to append

    Returns:
        Structured system prompt string.
    """
    role = _ARCHETYPE_ROLE.get(state.name, f"{state.who}·{state.where}·{state.when} agent")

    lines = [
        f"[ARCHETYPE: {state.name}]",
        f"WHO:      {state.who:<6}  →  {WHO_LABEL[state.who]}",
        f"WHERE:    {state.where:<6}  →  {WHERE_LABEL[state.where]}",
        f"WHEN:     {state.when:<6}  →  {WHEN_LABEL[state.when]}",
        ...
        f"  • Domain:       {WHERE_LABEL[state.where]}",
...
def build_minimal_prompt(state: State, op: Op) -> str:
    """Compact one-liner version for token-sensitive contexts."""
    return (
        f"You are a {state.name} agent "
        f"({WHO_LABEL[state.who]} / {WHERE_LABEL[state.where]} / {WHEN_LABEL[state.when]}). "
